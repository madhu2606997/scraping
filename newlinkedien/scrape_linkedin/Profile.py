...
class Profile(ResultsObject):
    ...
    def personal_info(self):
        ...
        #     'connected':'.pv-top-card--list-bullet > li',
        # })
        print(top_card)
        personal_info['name'] = top_card.find_element_by_css_selector(".pv-top-card--list > li").text.strip()
        personal_info['headline'] = top_card.find_element_by_css_selector(".flex-1.mr5 h2").text.strip()
        personal_info['location'] = top_card.find_element_by_css_selector(".pv-top-card--list-bullet > li").text.strip()
        personal_info['connected'] = top_card.find_element_by_css_selector(".pv-top-card--list-bullet > li:nth-child(2) >span").text.strip()
        ...
        image_element = one_or_default(top_card, '.pv-top-card__image > img')
        ...
        personal_info['image'] = image_element

        followers_text = text_or_default(self.soup,'.pv-recent-activity-section__follower-count', '')
        ...
        # }))
        personal_info['email'] = contact_info.find_element_by_css_selector('.ci-email.pv-contact-info__ci-container').text.strip();
        personal_info['phone'] = contact_info.find_element_by_css_selector('.ci-phone.pv-contact-info__ci-container').text.strip();
